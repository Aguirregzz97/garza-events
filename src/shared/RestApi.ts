  ...
  if (!shouldMockApis()) {
    ...
    if (response.status === 200) {
      if (newStatus == Status.CANCELLED) {
        await successDialog('Evento cancelado de manera exitosa')
      } else if(newStatus == Status.PENDING) {
        await successDialog('Evento cambiado a pendiente de manera exitosa!')
      } else {
        await successDialog('Evento aceptado de manera exitosa!')
      }
      redirectToEvents()      
    } else {
